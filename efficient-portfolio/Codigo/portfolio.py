...
def first_bisection(cov, mu, indx_assets0, indx_assets1, num_assets, p=1e-4):
    ...
    w = op.gradient_conjugated(np.array(cov), np.array(mu)+lamb, dim = num_assets )
...
#------------------------------------------------------------------------------
def hrp(cov, mu, corr, dendogram=False, sharpe=False):
...
#    plt.bar(range(num_assets), w, color='red' )
    ...
    if sharpe: return vol, ret
    else: return ret/vol #Utility(ret,var) #ret/vol 

#------------------------------------------------------------------------------
def find_max_portfolio(selected, cov, mu, corr, dendogram=False, sharpe=False):
    ...
    return hrp(new_cov, new_mu, new_corr, dendogram, sharpe)
...
def main():
    ...
    vol, ret = find_max_portfolio(selected, cov, mu, corr, False, True)
    print('retorno =', ret)
